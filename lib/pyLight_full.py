...
def highlight_matching_data(page, matched_values, type):
    ...
    for val in matched_values:
        ...
        # highlight.set_colors(stroke = fitz.utils.getColor('white'), fill = fitz.utils.getColor('red'))
...
def process_data(input_file: str, output_file: str, search_str: str, pages: Tuple = None, action: str = 'Highlight'):
    ...
    print(f"Liczba wyrazów {search_str} wynosi {total_matches} w pliku: {input_file}")
    # Save to output
...
def is_valid_path(path):
    ...
    """
    if not path:
        raise ValueError(f" Niewłaściwa ścieżka")
    if os.path.isfile(path):
        return path
    elif os.path.isdir(path):
        return path
    else:
        raise ValueError(f"Niewłaściwa ścieżka {path}")
